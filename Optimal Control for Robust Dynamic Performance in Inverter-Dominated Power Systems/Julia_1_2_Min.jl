...
for optim_iter in 1:Optim_iter
    ...
    global (_               , _                 , mnv_Vars_res      , _                 ,
            ...
            _               , mnv_y_theta       , mnv_n1_ratio      , mnv_n2_ratio      , _ , _ , mnv_max_emt_score , _ ) = read_results(BASE_SAVE_FOLDER*"//Optimization//Selected_Results//"*phase_read);   
    if (optim_iter==1)
...
if (keep_hope_for_configs == true)
    global Configs_Search = NLP_min;
else
    if (Optim_iter == 1)
        global Configs_Search = NLP_min;
    else
        global best_files   = readdir(BASE_SAVE_FOLDER*"//Plots//") 
...
for config_i in Configs_Search
    ...
    while (continue_mesh_refinement==true)
        ...
        while (msh_loop_stop_criteria==false)
            ...
            if primal_status(model_traj)==MOI.FEASIBLE_POINT
                global OPT_obj = value.(maximum([Vars[K_mnv*optim_iter,end] for optim_iter in 1:Optim_iter]))[1];
                ...
                if copy(OPT_obj)<=minimum(msh_loop_obj)
                    ...
                    global OPT_obj_res        = copy(value.(OPT_obj))       ;
                    # --------------- Save for plotting -------------------
