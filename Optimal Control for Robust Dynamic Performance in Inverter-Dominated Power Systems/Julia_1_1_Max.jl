...
if (Optim_iter == 1)
    global ( n1_ratio , n2_ratio , ibr_conf_num) = (n1_ratio_ini , n2_ratio_ini , ibr_conf_num_ini); # Initial guess: ALL_GFMs, n1 = 0.01, and n2 = 0.01
else
    global ( _ , _ , _ , _ , _ , _ , _ , _ , _ , _ , _ , ibr_conf_num , y_theta , n1_ratio , n2_ratio , _ ) = read_results(BASE_SAVE_FOLDER*"//Optimization//Selected_Results//"*string(Optim_iter-1)*"_Min_mnv_1");
end
load_ibrs(ibr_conf_num)

#********* Loop Initialization 1/2*********
global add_ED_to_model          = true  ; # true : Incorporates ED+AGC as constraints to the problem 
...
while (stop_criteria==false)
    ...
    global filename = BASE_SAVE_FOLDER*"//Warmstart_points//WS_Points_for_load_id_"*string(load_id)*"_and_jump_sense_"*jump_sense*".mat" ;
    ...
    if patience_slv < NLP_max
        println(repeat("*",100))
        println(repeat("*",100))
        println(repeat("*",44)," ", ms_count, " Big loop ",repeat("*",44))
        println(repeat("*",40)," Patience ", patience_slv, " out of ", NLP_max ,repeat("*",40))
        println(repeat("*",100))
        println(repeat("*",100)) 
    ...
    while (continue_mesh_refinement==true)
        ...
        while (msh_loop_stop_criteria==false)
            ...
            if primal_status(model_traj)==MOI.FEASIBLE_POINT
                global OPT_obj = value.(object_value)
                if (mesh_feasible_found == false) global msh_loop_obj = [OPT_obj]                   ;
                else                              global msh_loop_obj = push!(msh_loop_obj,OPT_obj) ;   end
                ...
                if copy(OPT_obj)>=maximum(msh_loop_obj)
                    ...
                    global OPT_obj_res        = copy(OPT_obj)               ;
                    # --------------- Save for plotting -------------------
                    global p_ref_res       = copy(value.(p_ref))                          ;
                    global q_ref_res       = copy(value.(q_ref))                          ;
                    global p_dist_res      = copy(value.(Pload_traj))                     ;
                    global q_dist_res      = copy(value.(Qload_traj))                     ;
                    global Pibr_res        = copy(value.(p_ibr))                          ;
                    global Qibr_res        = copy(value.(q_ibr))                          ;
                    global pload_res       = copy(value.(vN_D[:,load_buses] .* iL_D .+ vN_Q[:,load_buses] .* iL_Q)) ;
                    global qload_res       = copy(value.(vN_Q[:,load_buses] .* iL_D .- vN_D[:,load_buses] .* iL_Q)) ;
                ...
                println("("*phase*". model) Objective value: "    , OPT_obj ) ;  
            else
