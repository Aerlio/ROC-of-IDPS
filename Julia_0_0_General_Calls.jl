...
function custom_mesh(mesh_loop, phase , Optim_iter , ref_upd_time , H )
        if     (mesh_loop == 1)
                ...
                global break_times_mnv            =  cat(       range(0.0               , 1e-5                  ; length =  Int(round((        1e-5             )/1e-6  )+1) )[1:end-1]  ,
                                                                ...
                                                                range(1e-1              , ref_upd_time          ; length =  Int(round((   ref_upd_time - 1e-1   )/5e-2  )+1) )           ,
                                                                range(ref_upd_time+0.0  , ref_upd_time+1e-5     ; length =  Int(round((        1e-5             )/1e-6  )+1) )[1:end-1]  ,
        ...
        if phase == "Max"
                ...
                println("Length of mesh: ", length(break_times_mnv))
                return (break_times)
        else
                global break_times = repeat( copy(break_times_mnv) , Optim_iter ) ;
                println("Length of mesh: ", length(break_times))
        ...
        for n_L in 1:Nload
                if sum(Bload[:,n_L])!=0 
                        ...
                        elseif c_p[n_L]==1 && c_q[n_L]==1
                                global iL_D = cat(iL_D, @expression(model_traj,[k=1:K],(
                                                        (sum(c_p[n_L]*vN_D[k,Bload[:,n_L].==1])*Pload_traj[k,n_L] + c_q[n_L]*sum(vN_Q[k,Bload[:,n_L].==1])*Qload_traj[k,n_L])/
                                                                        (sum(vN_D[k,Bload[:,n_L].==1])^2+sum(vN_Q[k,Bload[:,n_L].==1])^2)
                                                        ) ), dims=[1,2][min(n_L,2)]);
                                global iL_Q = cat(iL_Q, @expression(model_traj,[k=1:K],(
                                                        (sum(c_p[n_L]*vN_Q[k,Bload[:,n_L].==1])*Pload_traj[k,n_L] - c_q[n_L]*sum(vN_D[k,Bload[:,n_L].==1])*Qload_traj[k,n_L])/
                                                                        (sum(vN_D[k,Bload[:,n_L].==1])^2+sum(vN_Q[k,Bload[:,n_L].==1])^2)
                                                        ) ), dims=[1,2][min(n_L,2)]);
                        end
                else
...
function save_results(filename, BREAK_TIMES , T_INT, VARS_INT, DERIVS_INT, PLOAD_INI , PLOAD_FIN , QLOAD_INI , QLOAD_FIN , P_REF_ED , Q_REF_ED , P_REF_AGC , 
                                IBR_CONF_NUM, Y_THETA , N1_RATIO , N2_RATIO , GLOBAL_OBJ)
        ...
        write(results, "GLOBAL_OBJ"     , GLOBAL_OBJ    );
        close(results)
...
function read_results(filename)
        ...
        variables_of_interest=[ "BREAK_TIMES",  "T_INT", "VARS_INT", "DERIVS_INT",
                                ...
                                "IBR_CONF_NUM" , "Y_THETA" , "N1_RATIO","N2_RATIO", "GLOBAL_OBJ"]
        (       BREAK_TIMES , T_INT, VARS_INT, DERIVS_INT, PLOAD_INI , PLOAD_FIN , QLOAD_INI , QLOAD_FIN , P_REF_ED , Q_REF_ED , P_REF_AGC , IBR_CONF_NUM , Y_THETA , N1_RATIO , N2_RATIO , GLOBAL_OBJ) = [read(results,i) for i in variables_of_interest];
        close(results)
        println(filename," - RESULTS READED")
        return (BREAK_TIMES , T_INT, VARS_INT, DERIVS_INT, PLOAD_INI , PLOAD_FIN , QLOAD_INI , QLOAD_FIN , P_REF_ED , Q_REF_ED , P_REF_AGC , IBR_CONF_NUM , Y_THETA , N1_RATIO , N2_RATIO , GLOBAL_OBJ)
end
